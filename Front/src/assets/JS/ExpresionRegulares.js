export const expresionsRegulars = {
    ...
    passwordRegex : /^[A-Z][A-Za-z0-9*+/_.-]*$/, //Validar contraseña que comienza con una letra mayúscula y permite números y letras sin espacios:
    textoRegex : /^[a-zA-Z\s]{3,20}$/, // solo se pueden ingresar letras a partir de 3 caracteres
    textonumberRegex : /^[a-zA-Z0-9\s]{2,50}$/, // solo se pueden ingresar letras a partir de 3 caracteres
    maxtextonumberRegex : /^[a-zA-Z0-9\s]{5,100}$/, // solo se pueden ingresar letras a partir de 3 caracteres
    ...
    alphanumericRegex : /^[A-Za-z0-9]+$/, //Validar número y letras sin espacios:
...
export const Texto_Expression = (value,setHook) => {
    let valor = value
      if (!expresionsRegulars.textoRegex.test(valor)) {
        return (
            setHook({
              error : true,
